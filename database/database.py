...
# больше не показывает анкету с айди этого человека
...
async def delete_one_like(user_id):
    ...
    print(delete_one_like)
    if len(delete_one_like)>=2:
        deleted_one_like = str(delete_one_like[1])
        for i in range(2, len(delete_one_like)):
            deleted_one_like+=f",{delete_one_like[i]}"
        stop_or_continue = "cont"
    else:
        deleted_one_like = None
        stop_or_continue = "stop"
    print(deleted_one_like)
    cursor.execute("UPDATE like SET people_who_like = ? WHERE user_id = ?", (deleted_one_like, user_id,))
    conn.commit()
    return stop_or_continue

# Дергаем id последней анкеты
async def get_last_ank_id(id):
    return cursor.execute("SELECT last_ank_id FROM like WHERE user_id={key}".format(key=id)).fetchone()[0]